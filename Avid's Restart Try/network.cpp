...
bool send(long proID, LSPMessage *msg, char* host){
    CLIENT *clnt;
    enum clnt_stat retval_1;
    
    #ifndef DEBUG
        clnt = clnt_create (host, PING, ALPHA, "udp");
        if (clnt == NULL) {
        ...
        retval_1 = clnt_call(clnt, proID, (xdrproc_t) xdr_message, (caddr_t) argp,(xdrproc_t) xdr_void, (caddr_t) clnt_res, TIMEOUT);
...
bool network_send_message(Connection *conn, LSPMessage *msg){
    ...
    if(send(conn->proID, *msg, conn->host)){
